...
_JavaScript_ 是一门基于原型的语言，**对象的是基于原型对象产生的**。

## 原型

对象的构造函数的 `prototype` 属性和对象的 `[[prototype]]` 内部插槽指向同一个对象，称为实例的原型。
过去使用对象的`__proto__`属性表示原型对象，该特性已从标准中移除，可以使用`Object.getPrototypeOf()`来获取实例的原型对象。
...
1. 访问对象的属性或方法时，如果对象内部不存在，JS 引擎会沿着原型链向上查找，直到找到或到达原型链的顶端
...
1. 由于原型链的查找机制，数组可以调用 Array.prototype 上的方法
2. Vue 2 中使用 Vue.use 安装插件，比如 Vue.use(Router)，可以在全局使用 this.$router
